<template>
  <div class="fstr-content">
    <div class="fstr-card">
      ...
      <div class="p-2 flex">
        <input v-model="searchQuery" type="text" placeholder="Search Accounts"
      ...
      <teleport to="body">
        <div v-if="isModalOpen"
          ...
          <div
            ...
            <AccountForm @closeModal="closeModal" v-model="selectedAccount"/>
            ...
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tr v-for="(account, index) in filteredAccounts" :key="index"
                ...
                <td class="px-2 py-1 whitespace-normal">
                  <div class="text-xs text-gray-900">{{ account.name }}</div>
                </td>
                <td class="px-2 py-1 whitespace-normal break-all">
                  <div class="text-xs text-gray-900">{{ account.shipping_address }}</div>
                </td>
                <td class="px-2 py-1 whitespace-normal">
                  <div class="text-xs text-gray-900">{{ account.phone_number }}</div>
                </td>
              </tr>
              <!-- End repeat -->
              </tbody>
...
export default {
      ...
      try {
        ...
        return this.accounts.filter(account => {
          ...
          return account.name?.toLowerCase().includes(searchQuery) ||
              account.shipping_address?.toLowerCase().includes(searchQuery) ||
              account.phone_number?.toLowerCase().includes(searchQuery);
        });
      } catch (e) {
        console.error(e);
        return this.accounts;
  ...
  async created() {
    this.accounts = await this.$store.dispatch('getAccounts');
  },
};
</script>
